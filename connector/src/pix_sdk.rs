...
pub use curvy_sdk::Account;
use curvy_sdk::{CurvyClient, Identity, OwnedNote, PreparedDeposit, Route, ScanRecipient, TxLedger, ViewerIdentity};
use hopr_api::types::{
...
pub struct RsSdkCurvyAdapterConfig {
    ...
    pub spender: Account,
    /// EVM key that funds shield portals and submits allocation and withdrawal calls.
    pub submitter_private_key: String,
    /// Curvy relayer/operator EVM key used for role-gated portal deployment and
    /// pending-note commitment.
    pub operator_private_key: String,
    /// Token identifier used by all pool notes.
    pub token: u64,
    /// Public asset used to fund the deterministic shield portal.
    pub funding_asset: CurvyFundingAsset,
...
/// Asset transferred to the deterministic portal before it is deployed.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum CurvyFundingAsset {
    Native,
    Erc20(String),
}
...
impl RsSdkCurvyAdapterConfig {
    pub fn new(spender: Account, submitter_private_key: String, operator_private_key: String, token: u64) -> Self {
        Self {
            ...
            token,
            funding_asset: CurvyFundingAsset::Native,
            route: Route::Blokli,
            fee_recipient: None,
        }
    }

    /// Fund shield portals with the given ERC-20 rather than native currency.
    pub fn with_erc20_funding(mut self, token_address: impl Into<String>) -> Self {
        self.funding_asset = CurvyFundingAsset::Erc20(token_address.into());
        self
    }
}
...
impl StoredAllocation {
    fn new(
        ...
        Ok(Self {
            ...
            scan_key: scan_key.as_ref().to_vec(),
...
impl RsSdkCurvyAdapter {
        ...
        let prepared = shield.prepared()?;
        {
            ...
            if !matches!(observed_status, 1 | 2) {
                if shield.stage == StoredShieldStage::Prepared {
                    let portal_balance = match &self.config.funding_asset {
                        CurvyFundingAsset::Native => self.client.eth_balance(&prepared.portal_address).await?,
                        CurvyFundingAsset::Erc20(token) => {
                            self.client.erc20_balance(token, &prepared.portal_address).await?
                        }
                    };
                    if portal_balance == 0 {
                        ledger.push(match &self.config.funding_asset {
                            CurvyFundingAsset::Native => {
                                self.client
    ...
    fn recipient(
        ...
        let viewer = ViewerIdentity::new(
            format!("{}.{}", U256::from_big_endian(&kx), U256::from_big_endian(&ky)),
            format!("{}.{}", U256::from_big_endian(&vx), U256::from_big_endian(&vy)),
        )?;
...
impl CurvySdkAdapter for RsSdkCurvyAdapter {
    ...
    async fn deposit(
        ...
        self.allocate(&[(id, dst, scan_key, amount)]).await
    }
...
mod tests {
    ...
    use hopr_api::types::{
        ...
        primitive::prelude::{IntoEndian, U256},
    };
    ...
    fn scan_key(spend_private_key: &str, view_private_key: &str) -> anyhow::Result<CurvyScanPublicKey> {
        let (spend_meta_key, view_public_key) = curvy_core::stealth::get_meta(spend_private_key, view_private_key)
        ...
        Ok(CurvyScanPublicKey::from_affine_coordinates((&kx, &ky), (&vx, &vy))?)
    }
